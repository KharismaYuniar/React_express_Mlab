...
class App extends Component {
  constructor(){
    super();
    this.state={nama:'',usia:'',kota:'',toko:[]}
  }
  klik(){
    this.setState({
  ...
  render() {
    const data = this.state.toko.map((item, index)=>{
      var id   = [item.id]
      var nama = [item.nama]
      var usia = [item.usia]
      var kota = [item.kota]
      return <tr key={index}><td>{id}</td><td>{nama}</td><td>{usia}</td><td>{kota}</td></tr>;
    })
    return (
      <div>
      <center>
      <h1>REACT - EXPRESS - MLAB</h1><br/>  
      <h1>DATA KARYAWAN</h1>
      <div className="row">
